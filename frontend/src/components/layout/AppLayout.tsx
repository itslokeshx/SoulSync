...
export function AppLayout() {
  ...
  const {
    ...
    currentTime, duration, togglePlay, setVolume, toggleShuffle,
    cycleRepeat, playSong, seekTo, pause
  } = usePlayerStore();

  const {
    queue, queueIndex, next, prev, move, remove, shuffle, setQueue
  } = useQueueStore();
  ...
  const handleLike = useCallback((song: any) => {
    gate(() => {
  ...
  const appContextValue = {
    ...
    playSong: (song: any, songs?: any[]) => {
      if (songs) setQueue(songs, songs.findIndex(s => s.id === song.id));
      playSong(song);
    },
    likedSongs,
    handleLike,
    setLikedSongs: () => { }, // No-op, managed by hook
    ...
    <AppContext.Provider value={appContextValue}>
      <div className="flex h-screen bg-sp-black overflow-hidden font-sans">
        ...
        >
          {/* Top Header */}
          {!isSearchPage && (
            <header className="flex-shrink-0 flex items-center gap-4 px-4 md:px-6 py-3 sticky top-0 z-20 backdrop-blur-3xl"
              style={{ background: `linear-gradient(to bottom, ${bgColor}40 0%, transparent 100%)` }}
            >
              <button onClick={() => navigate("/")} className="flex items-center gap-2 md:hidden">
                ...
                <span className="text-lg font-bold text-white tracking-tight">SoulSync</span>
              </button>

              <div className="hidden md:flex gap-2">
                <button onClick={() => navigate(-1)} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-white/10 transition-colors">
                  <ChevronLeft size={18} />
                </button>
              </div>
              ...
              <div className="flex-1 relative max-w-xl">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    navigate(e.target.value.trim() ? `/search?q=${encodeURIComponent(e.target.value)}` : "/search");
                  }}
          ...
          {/* Main Content */}
          <main className={`flex-1 overflow-y-auto ${!isSearchPage ? "px-4 md:px-8 pb-32 md:pb-24" : ""}`}>
            <Outlet />
          </main>
        </div>
        ...
        {queueOpen && currentSong && (
          <QueuePanel
            ...
            onJump={(idx) => { const s = queue[idx]; if (s) { setQueue(queue, idx); playSong(s); } }}
            onMove={move}
            onRemove={remove}
          />
        )}
        ...
        <PlayerBar
          ...
          onSeekEnd={() => { }}
          onVolume={setVolume}
          onPrev={() => { const p = prev(); if (p) playSong(p); }}
          onNext={() => { const n = next(repeatMode); if (n) playSong(n); }}
