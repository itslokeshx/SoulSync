...
export function DuoPanel({ onSendMessage, onEndSession }: DuoPanelProps) {
  const {
    panelOpen,
    setPanelOpen,
    roomCode,
      ...
      <div className="flex gap-0.5 mx-4 md:mx-5 mt-2.5 md:mt-3 mb-2 p-0.5 rounded-lg bg-white/[0.03]">
        {[
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-[11px] font-semibold transition-all ${activeTab === id ? "bg-white/[0.08] text-white" : "text-sp-sub/50 hover:text-white"}`}
          >
            <Icon size={12} />
            {label}
          </button>
                ...
                messages.map((m, i) => {
                  ...
                  return (
                    <div key={i}>
                      ...
                      >
                        <div
                          className={`relative max-w-[80%] px-3.5 py-2 rounded-2xl text-[13px] leading-relaxed ${isMe ? "bg-sp-green/20 text-sp-green rounded-br-md" : "bg-white/[0.06] text-white/85 rounded-bl-md"}`}
                        >
                          {!isMe && (
