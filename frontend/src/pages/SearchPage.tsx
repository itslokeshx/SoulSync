...
function SongRow({ song, onPlay }: { song: any; onPlay: (s: any) => void }) {
  ...
  return (
    <button
      onClick={() => onPlay(song)}
      className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/[0.06] active:bg-white/[0.09] transition-all group text-left"
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-white text-[14px] font-medium truncate">{song.name}</p>
        <p className="text-white/40 text-[12px] truncate">{artist}</p>
      </div>
      {duration && <span className="text-white/25 text-[11px] flex-shrink-0">{duration}</span>}
    </button>
  );
}

function TopResultCard({ song, onPlay }: { song: any; onPlay: (s: any) => void }) {
  const img = bestImg(song.image) || FALLBACK_IMG;
  const artist = getArtists(song);
  return (
    <button
      onClick={() => onPlay(song)}
      className="w-full flex flex-col items-start p-4 rounded-2xl bg-white/[0.05] hover:bg-white/[0.09] border border-white/[0.06] active:scale-[0.98] transition-all group"
    >
      ...
      <p className="text-white text-[14px] font-bold truncate w-full text-left">{song.name}</p>
      <p className="text-white/40 text-[11px] truncate w-full text-left mt-0.5">{artist}</p>
...
function ArtistChip({ artist }: { artist: any }) {
  const img = artist.image?.[2]?.link || artist.image?.[1]?.link || artist.image?.[0]?.link || FALLBACK_IMG;
  return (
    <div className="flex flex-col items-center gap-2 w-20 flex-shrink-0">
      <img
        ...
        onError={(e) => { (e.currentTarget as HTMLImageElement).src = FALLBACK_IMG; }}
      />
      <p className="text-white/60 text-[11px] text-center font-medium truncate w-full">{artist.name}</p>
    </div>
  );
}
...
function AlbumCard({ album }: { album: any }) {
  ...
  return (
    <div className="flex flex-col gap-2 w-32 flex-shrink-0">
      ...
      />
      <div>
        <p className="text-white/80 text-[12px] font-medium truncate">{album.name}</p>
        <p className="text-white/35 text-[11px] truncate">{album.primaryArtists || ""}</p>
      </div>
    </div>
  );
}
...
export function SearchPage() {
  ...
  const { query, setQuery, state, isStale, result, loadingMore, loadMore } = useSearch();
    ...
    <div className="h-full flex flex-col bg-[#050505]">
      ...
      >
        <div className="flex items-center gap-2 max-w-2xl">
          ...
          <div className="flex-1 flex items-center gap-2.5 h-12 px-4 rounded-2xl bg-white/[0.08] border border-white/[0.09] focus-within:border-sp-green/60 focus-within:bg-white/[0.11] transition-all duration-150">
            ...
            {state === "loading" ? (
              <Loader2 size={16} className="text-sp-green flex-shrink-0 animate-spin" />
            ) : isStale ? (
      ...
      <div className="flex-1 overflow-y-auto thin-scrollbar px-3 md:px-5 pb-28">
                  ...
                  {recentSearches.slice(0, 6).map((r) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Clock size={13} className="text-white/20 group-hover:text-white/40 transition-colors" />
                        <span className="text-white/65 text-[14px]">{r}</span>
                      </div>
                      <ArrowUpRight size={13} className="text-white/15 group-hover:text-sp-green/60 transition-colors" />
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div>
              ...
              <div className="space-y-0.5">
                {TRENDING.map((t) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Search size={13} className="text-white/20 group-hover:text-white/40 transition-colors" />
        ...
        {state === "results" && result && (
          <div className={`py-4 space-y-7 max-w-2xl transition-opacity duration-200 ${isStale ? "opacity-60" : "opacity-100"}`}>
            ...
            {result.artists?.length > 0 && (
              <div>
                ...
                <div className="flex gap-5 overflow-x-auto pb-1 thin-scrollbar">
                  {result.artists.slice(0, 8).map((a: any) => <ArtistChip key={a.id || a.name} artist={a} />)}
                </div>
              </div>
            )}
            ...
            {result.albums?.length > 0 && (
              <div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-1 thin-scrollbar">
                  {result.albums.slice(0, 8).map((a: any) => <AlbumCard key={a.id || a.name} album={a} />)}
                </div>
              </div>
            )}
        ...
        {state === "no-results" && (
          <div className="flex flex-col items-center justify-center py-24 px-4 text-center">
            ...
            <p className="text-white/50 font-semibold text-base">No results for "{query}"</p>
            <p className="text-white/25 text-sm mt-2">Try different words or a mood</p>
        ...
        {state === "error" && (
          <div className="flex flex-col items-center justify-center py-24 text-center px-4">
            <p className="text-white/40 text-sm">Search failed. Check your connection.</p>
            <button
