...
export default function ImportPage() {
  ...
  const toSongEmbed = (song: any) => {
    ...
    const artistName =
      artists.map((a: any) => a.name).join(", ") || song.subtitle || "";
  ...
  const handleAIMatch = async () => {
    ...
    setError("");
    try {
      const res = await api.buildPlaylistFromImport(
        preview.songNames,
        preview.name,
      );
      ...
      if (songs.length > 0) {
        ...
        setMatchResult(res);
        setError(
          "No songs could be matched on JioSaavn. Try pasting names manually.",
        );
      }
    } catch (err: any) {
    ...
    <div className="pb-32 overflow-x-hidden min-h-screen">
      ...
      <div className="flex items-center gap-3 px-4 sm:px-5 pt-4 sm:pt-5 pb-3 sm:pb-4">
        ...
        <div className="min-w-0">
          ...
          <p className="text-white/30 text-[11px] sm:text-[12px] truncate">
            From any platform — free, no account needed
          </p>
        </div>
      </div>

      {/* ─── PLATFORM CHIPS ─── */}
      <div className="flex gap-2 px-4 sm:px-5 mb-4 sm:mb-6 overflow-x-auto no-scrollbar max-w-full">
        {PLATFORMS.map((p) => (
          <button
            key={p.id}
            onClick={() => {
            ...
            className={`flex-shrink-0 flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl border transition-all duration-200 active:scale-95 ${
              selected === p.id
          ...
          <div className="bg-sp-green/[0.08] border border-sp-green/20 rounded-2xl p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sp-green text-sm font-medium">
                  {matchResult.stats?.found || 0} of{" "}
                  {matchResult.stats?.total || 0} songs found
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <button
                onClick={() =>
                  navigate(
                    matchResult.savedPlaylistId
                      ? `/playlist/${matchResult.savedPlaylistId}`
                      : "/library",
                  )
                }
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/40 text-sm">
                  {preview.count} songs found
                </p>
                <p className="text-sp-green text-[11px] font-medium mt-0.5 flex items-center gap-1">
                  <Check size={12} /> Imported from{" "}
                  {preview.detectedPlatform.replace("_", " ")}
                </p>
              </div>
            </div>
            ...
            <div className="mx-5 mb-4 bg-red-500/[0.08] border border-red-500/20 rounded-xl px-4 py-3">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  {suggestion && (
                    <p className="text-white/30 text-xs mt-1">{suggestion}</p>
                  )}
                </div>
              </div>
            </div>
              ...
              {[
                ...
                <div key={i} className="flex items-start gap-3">
                  <div className="w-6 h-6 rounded-full bg-sp-green/10 flex items-center justify-center flex-shrink-0">
                    <span className="text-sp-green text-xs font-bold">
                      {i + 1}
                    </span>
                  </div>
